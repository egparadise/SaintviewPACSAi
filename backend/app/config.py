...
# ══════════════════════════ 단일 워커 배포 계약 ══════════════════════════
# 이 백엔드는 **단일 워커 전용**이다. 캐시·락·인플라이트·리스너가 전부 프로세스
# 인메모리(모듈 전역 dict)라 워커가 2개 이상이면 조용히 깨진다:
#   · security_service._fail_counts            → 로그인 실패 잠금 임계값이 실질 N배(브루트포스 방어 약화)
#   · htj2k_stream._inflight_series/_TS_*      → 같은 시리즈를 워커 수만큼 중복 프리인코딩
#   · htj2k 캐시 상한(CACHE_MAX_MB)            → 워커마다 따로 세어 디스크 상한이 실질 N배
#   · worklist._TREE_CACHE / STT 모델          → 상한이 워커 수만큼 곱해져 OOM
#   · MPPS/MWL/MLLP 리스너 · ai_worker · 백업  → 2번째 워커부터 bind 실패 / 같은 잡을 중복 실행
MULTI_WORKER_MESSAGE = (
    "다중 워커 금지 — 이 백엔드는 캐시·락·인플라이트·리스너가 모두 프로세스 인메모리라 "
    "워커마다 따로 돌아 조용히 깨진다(로그인 실패 잠금 임계값 N배 / 같은 시리즈 중복 인코딩과 "
    ".part 파일 동시 기록으로 캐시 손상 / 디코드·인코드 캐시와 STT 모델이 워커 수만큼 곱해져 OOM / "
    "MPPS·MWL 포트 bind 실패 / ai_worker·백업 잡 중복 실행). "
    "지금 바로 할 일: --workers 1 (gunicorn 은 -w 1)로 띄우고 WEB_CONCURRENCY 를 지워라. "
    "정말 워커를 늘리려면 먼저 상태를 프로세스 밖으로 빼야 한다 — 실패카운터는 DB/Redis 로, "
    "인플라이트 락은 파일락/Redis 락으로, 인코드 캐시는 공유 저장소로, "
    "MPPS·MWL·MLLP·DDNS·ai_worker 는 별도 단일 프로세스로 분리."
)
...
class Settings:
    ...
    # 왜 기본을 뒤집었나: /api/signup 은 무인증으로 병원 + 그 병원의 **관리자(role=admin)**
    # 계정을 즉석 생성한다. 기본이 켜져 있으면 익명 공격자가 '가입→로그인' 두 번으로
    # 유효한 관리자 자격증명을 스스로 발급받는다(인증이 셀프서비스 = 실질 무인증).
    ...
    def validate_for_prod(self) -> None:
        ...
        if self.signup_enabled:
            problems.append(
                ...
                "무인증 가입은 '가입→로그인' 두 번으로 병원 관리자 자격증명을 스스로 발급한다. "

...
⚠ 왜 즉시 활성이 아닌가 — 이 엔드포인트는 의존성이 get_db 뿐인 **무인증** 경로다.
  예전에는 여기서 만든 계정이 곧바로 로그인 가능했고, 그 계정의 role 은 admin 이다.
  즉 익명 공격자가 `POST /api/signup` → `POST /api/auth/login` 두 번으로 유효한
  병원 관리자 자격증명을 스스로 발급받았다(인증이 셀프서비스 = 실질 무인증).
...
def signup(body: SignupBody, db: Session = Depends(get_db)):
    ...
    admin = Account(
        ...
        enabled=False,   # 승인 대기 — authenticate() 가 거부한다
    )
    db.add(admin)
    db.flush()

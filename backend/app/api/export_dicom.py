...
경로 규칙(표준 반출 관례):
    DICOM/<환자ID>/<검사일_모달리티_UID꼬리>/S<시리즈번호>/<일련번호>.dcm
환자별·검사별로 갈라 두어야 받는 쪽에서 섞이지 않는다.

⚠ 이 모듈이 옛 services/export_service.py 를 대체한다. 옛 경로는 ZIP 전체를 메모리에서
  만든 뒤 되감는 방식이라 **4MB 를 넘는 반출이 전부 깨져 있었다**(_ZipDrain 주석 참조).
"""
...
from app.api.deps import require_effective, require_effective_download
from app.db import get_db

router = APIRouter(prefix="/api/export", tags=["export"])

# 반출은 PHI 가 브라우저 밖으로 나가는 동작이다 — 병원별 매트릭스를 반영한 유효 권한을 쓴다
# (옛 POST /api/studies/export 와 동일한 게이트: image.print).
_EXPORT_PERM = "image.print"
...
def _orthanc_candidates(db: Session, hospital_id: int | None) -> list:
    """이 검사를 가지고 있을 수 있는 Orthanc 후보 — 병원 전용 먼저, 그 다음 공용."""
    from app.dicom.orthanc import OrthancClient, client_for_hospital

    out = []
    try:
        out.append(client_for_hospital(db, hospital_id))
    except Exception:  # noqa: BLE001 — 레지스트리 해석 실패는 공용만으로 진행(가용성 우선)
        pass
    out.append(OrthancClient())
    return out
...
def _entries(db: Session, study_id: int, user: dict) -> tuple[dict, list[dict]]:
    ...
    ⚠ 병원 스코프 가드가 여기 있다 — 반출은 검사 여러 건을 한 번에 받으므로
      **건마다** 확인해야 한다(한 건이라도 새면 그대로 유출이다).
    """
    from app.api.worklist import cached_series_tree_multi
    from app.dicom.orthanc import OrthancClient, client_for_hospital
    ...
    if not is_sys_admin and user.get("hid") and st.hospital_id != user.get("hid"):
        raise HTTPException(status_code=404, detail=f"검사를 찾을 수 없습니다: {study_id}")
    ...
    if not st.orthanc_id:
        return d, []
    # 병원별 Orthanc 컨테이너 우선, 없으면 공용으로 폴백 — 배포에 따라 보관 위치가 다르다.
    # (폴백이 캐시 때문에 무력화되지 않게 하는 규칙은 cached_series_tree_multi 주석 참조)
    series = cached_series_tree_multi(_orthanc_candidates(db, st.hospital_id), st.orthanc_id)
...
def _read(db: Session, f: dict) -> bytes:
    """DICOM 원본 바이트 — 트리를 준 곳과 같은 후보 순서로 찾는다."""
    last: Exception | None = None
    for c in _orthanc_candidates(db, f.get("hospital_id")):
        try:
            return c.instance_file(f["orthanc_id"])
        except Exception as e:  # noqa: BLE001 — 이 후보에 없다 — 다음 후보로
            last = e
    raise last or RuntimeError("영상을 가진 Orthanc 를 찾지 못했습니다")
...
def manifest(study_ids: str = Query(...), db: Session = Depends(get_db),
             user: dict = Depends(require_effective(_EXPORT_PERM))):
    """내보낼 파일 목록 — 폴더/USB 저장은 이 목록으로 한 장씩 받아 기록한다(진행률 표시)."""
    ids = _ids(study_ids)
    collected = [_entries(db, sid, user) for sid in ids]
    _dedupe(collected)
    studies = []
    total = 0
    for sid, (d, files) in zip(ids, collected):
        total += len(files)
        studies.append({
...
def one_file(study_id: int, sop_uid: str, db: Session = Depends(get_db),
    ...
    return StreamingResponse(io.BytesIO(data), media_type="application/dicom",
                             headers={"Content-Disposition":
                                      f'attachment; filename="{f["path"].split("/")[-1]}"'})
...
def package(study_ids: str = Query(...), format: str = "zip",
            ...
            user: dict = Depends(require_effective_download(_EXPORT_PERM))):
    """ZIP(스트리밍) 또는 CD 굽기용 ISO 이미지.

    인증은 download_user 계열 — 브라우저 내려받기는 Authorization 헤더를 못 붙이므로
    ?token= 도 받는다(검증·권한 판정은 헤더 경로와 완전히 동일).
    """
    ids = _ids(study_ids)

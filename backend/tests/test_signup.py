...
⚠ 공개 가입은 **기본 OFF** 다(app/config.py: SAINTVIEW_SIGNUP_ENABLED 기본 0). 무인증
  /api/signup 이 즉시 쓸 수 있는 **관리자** 계정을 만들면 그것만으로 인증이 셀프서비스가
  되기 때문이다. 이 파일의 테스트는 그 스위치를 명시적으로 켜고(signup_on) 가입 자체의
  계약을 검증한다.
"""
...
def test_public_signup_is_off_by_default(client):
    """기본값 자체가 방어선이다 — 스위치를 켜지 않으면 무인증 가입이 아예 없다."""
    from app.config import get_settings

    assert get_settings().signup_enabled is False, "공개 자가가입이 기본으로 켜져 있다"
    assert client.post("/api/signup", json=_payload(username="atk_off", hosp="기본꺼짐병원")).status_code == 403

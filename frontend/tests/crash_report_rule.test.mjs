 ...
 *   ② build 가 늘 "" 였다 — 어느 빌드에서 난 오류인지 알 수 없었다.
 *      (여기서는 lib/version.ts 의 APP_VERSION·APP_RELEASE_DATE 를 그대로 찍는다)
...
test("XHR 거부 — status 와 URL 이 메시지에 남는다 (핵심 회귀 방어)", () => {
  const xhr = {
    ...
    responseURL: "https://localhost:5173/api/htj2k/studies/1.2.3/series/1.2.4/instances/1.2.5/frames/1",
  };
  ...
  assert.match(d.message, /studies\/1\.2\.3/, `주소가 없다: ${d.message}`);
});
...
test("fetch Response 도 status·URL 을 남긴다", () => {
  const d = describeReason({ ok: false, status: 503, url: "https://localhost:5173/api/auth/client-login" });
  assert.match(d.message, /503/);
  assert.match(d.message, /client-login/);
});

test("일반 객체 — message/error/detail 중 있는 것을 쓴다", () => {
  assert.match(describeReason({ message: "권한 없음" }).message, /권한 없음/);
  assert.match(describeReason({ detail: "Orthanc 응답 없음" }).message, /Orthanc/);

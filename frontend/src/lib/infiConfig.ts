...
export const IN_PALETTE: { id: string; icon: string; label: string; impl: boolean; mode?: boolean }[] = [
  ...
  { id: "magnify", icon: "⌕", label: "Magnification — 마우스를 올리면 부분 확대경", impl: true, mode: true },
  ...
  { id: "cursor3d", icon: "✛", label: "3D Cursor — 클릭 지점을 모든 페인에서 동일 위치로 표시·이동", impl: true, mode: true },
  { id: "dictation", icon: "🎙", label: "Dictation — 음성 녹음 시작/정지 (세션 보관)", impl: true },
  { id: "playdict", icon: "🔊", label: "Play Dictation — 녹음 재생", impl: true },
  { id: "refreshExam", icon: "🔄", label: "Refresh Exam — 검사 정보 갱신", impl: true },
  // p.12 중단 — 선택/방향
  { id: "selAll", icon: "⊞", label: "Select All — 모든 페인 선택 (A)", impl: true },
  { id: "selInv", icon: "⊟", label: "Select All Inverse — 선택 반전", impl: true },
  ...
  { id: "shutEl", icon: "◙", label: "Ellipse Shutter — 두 점(박스)으로 타원 밖 가림", impl: true, mode: true },
  { id: "shutRect", icon: "▣", label: "Rectangle Shutter — 두 점으로 사각 밖 가림", impl: true, mode: true },
  { id: "shutPoly", icon: "⬠", label: "Polyline Shutter — 점 클릭, 더블클릭 완료", impl: true, mode: true },
  // p.13 상단 — 필터/스크롤
  ...
  // p.13 측정/분석
  { id: "ctr", icon: "♥", label: "CT Ratio 심흉비 — 심장 2점 + 흉곽 2점", impl: true, mode: true },
  { id: "limb", icon: "🦵", label: "Limb Length — 선1(2점)+선2(2점) 길이 비교", impl: true, mode: true },
  { id: "centerline", icon: "╂", label: "Center Line — 두 선(4점)의 중앙선 표시", impl: true, mode: true },
  { id: "profile", icon: "📈", label: "Profile — 두 점 선의 픽셀값 그래프", impl: true, mode: true },
  { id: "table2d", icon: "▤", label: "2D Table — 두 점 영역의 픽셀값 표", impl: true, mode: true },
  { id: "calibrate", icon: "📐", label: "Calibrate — Pixel Spacing 정보", impl: true },
  { id: "spine", icon: "🦴", label: "Spine Label — 클릭마다 라벨(첫 클릭에 시작 라벨 입력)", impl: true, mode: true },
  { id: "anno3d", icon: "🧊", label: "3D 뷰어 열기 — Crosshair 로 3D 위치 표시", impl: true },
  // p.13~14 — 2D 주석/측정
  { id: "arrow2d", icon: "↗", label: "2D Arrow — 꼬리→머리 두 점", impl: true, mode: true },
  { id: "text2d", icon: "T", label: "2D Text — 클릭 후 문구 입력", impl: true, mode: true },
  { id: "box2d", icon: "▭", label: "2D Box 메모 — 두 점 + 제목 입력", impl: true, mode: true },
  { id: "key2d", icon: "🔑", label: "Key — 현재 이미지를 키이미지로 등록/해제 (워크리스트 🔑·Key 필터 조회)", impl: true },
  { id: "circle", icon: "◯", label: "Circle — 중심→가장자리 두 점", impl: true, mode: true },
  { id: "polyline", icon: "〰", label: "Polyline — 점 클릭, 더블클릭 완료", impl: true, mode: true },
  ...
  { id: "mellipse", icon: "⬭", label: "Measure Ellipse ROI — 두 점(박스), 평균/편차/면적", impl: true, mode: true },
  { id: "mrect", icon: "⬜", label: "Measure Rect ROI — 두 점, 평균/편차/면적", impl: true, mode: true },
  { id: "cobb", icon: "⟁", label: "Cobb Angle — 선1(2점)+선2(2점) 사이 각", impl: true, mode: true },
  { id: "marking", icon: "M", label: "Marking — 클릭 후 짧은 표기 입력", impl: true, mode: true },
  { id: "lens", icon: "🎯", label: "Lens — 클릭 지점 픽셀값(근사 HU)", impl: true, mode: true },

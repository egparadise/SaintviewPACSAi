...
export default function App() {
  ...
  const [collabOpen, setCollabOpen] = useState(false);   // 다학제 협진 — 친구·메신저 패널

  // 다학제 협진 WebSocket — 세션이 있으면 붙고 없으면 끊는다. **창마다 소켓 1개**가 설계다
  // (다중 모니터에서 각 뷰어 창이 독립적으로 미러를 주고받아야 하므로 — lib/collab.ts 주석).
  // ⚠ 이 effect 는 IS_VIEWER_WINDOW 조기 반환보다 **위**에 있어야 뷰어·판독 창도 덮인다.
  useEffect(() => {

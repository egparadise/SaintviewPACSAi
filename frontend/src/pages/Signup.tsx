    ...
    <div style={{ height: "100%", overflow: "auto", display: "grid", placeItems: "center", padding: 20 }}>
      <div style={{ width: 720, maxWidth: "100%", display: "flex", flexDirection: "column", gap: 14,
        ...
        <Section title={tr("병원 정보")}>
          ...
          {show("departments") && <Field label="진료과 (콤마 구분)" req={reqd("departments")}><input style={inp} placeholder={tr("영상의학과,내과")} value={f.departments} onChange={(e) => set("departments", e.target.value)} /></Field>}
          {show("address") && (
            <div style={{ gridColumn: "1 / -1", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <Field label="주소" req={reqd("address")}><input style={inp} value={f.address} onChange={(e) => set("address", e.target.value)} placeholder={tr("주소 검색 또는 직접 입력")} /></Field>
              <Field label="상세주소"><input style={inp} value={f.address_detail} onChange={(e) => set("address_detail", e.target.value)} placeholder={tr("동·호수 등 상세주소 직접 입력")} /></Field>
            </div>
          )}
        ...
        <div style={{ fontSize: 11, color: "var(--text-secondary)" }}>{tr("⚠ 주민번호는 앞 6자리(생년월일)만 저장되며, 카드는 마지막 4자리만 보관됩니다. 전체 번호는 저장하지 않습니다.")}</div>
      </div>
    </div>
  );
}
